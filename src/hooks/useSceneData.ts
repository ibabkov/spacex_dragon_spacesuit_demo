...
import { MODEL_URL, SKYBOX_TEXTURE_URL_2 } from '../constants/scenes/spaceShip';
...
export function useSceneData(): ISceneData | null {
  ...
  const getSceneData = React.useCallback(async () => {
    ...
    const [model, skyboxTexture] = await Promise.all([
      modelLoader.loadAsync(MODEL_URL),
      skyboxLoader.loadAsync(SKYBOX_TEXTURE_URL_2 as any),
    ]);
