...
export function useModelAnimate(
  ...
  useFrame(() => {
    ...
    const position =
      getModelAnimationProgress(time, MODEL_POSITION_ANIMATION_PERIOD) *
      MODEL_POSITION_ANIMATION_SCALE;
    const rotation =
      getModelAnimationProgress(time, MODEL_ROTATION_ANIMATION_PERIOD) *
      MODEL_ROTATION_ANIMATION_SCALE;

    ref.current.position.set(
      -position,
      position + MODEL_POSITION_Y_SHIFT,
      -position
    );
